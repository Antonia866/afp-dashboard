...
# Hojas esperadas (con fallback en load_* si tu archivo usa variantes)
...
def _find_last_date_override_from_I2(xls_source, sheet_name: str) -> Optional[pd.Timestamp]:
    ...
    Lee última fecha desde Hola Valores!I2
      - Fila 2  -> index 1
      - Columna I -> index 8
    La celda viene como fecha Excel (ok).
    Se normaliza a fin de mes (MonthEnd(0)) por seguridad.
    """
    try:
...
def add_rules_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Flujo AFP intuitivo (activas/seguidoras)
    df["Flujo_AFP"] = "Sin señal clara"
    df.loc[(df["Delta_GAP"] > 0) & (df["Aceleracion"] > 0), "Flujo_AFP"] = "Entrada activas"
    df.loc[(df["Delta_GAP"] > 0) & (df["Aceleracion"] < 0) & (g["Fuerte_Expansion"].shift(1) == True), "Flujo_AFP"] = "Entrada seguidoras"
    ...
    # Etiquetas para gráfico (sin términos antiguos)
    df["Etiqueta_Entrada"] = np.where(df["Flujo_AFP"].isin(["Entrada activas", "Entrada seguidoras"]), df["Flujo_AFP"], "")
    df["Etiqueta_Salida"] = np.where(df["Flujo_AFP"].isin(["Salida activas", "Salida seguidoras"]), df["Flujo_AFP"], "")
...
def add_actions(df_model: pd.DataFrame) -> pd.DataFrame:
    ...
    def tactical(r):
        ...
        if pd.notna(p) and pd.notna(fs):
            ...
            if (p >= 0.60) and (sem == "🟢"):
                return "BUY (light)", "Prob alta + 🟢 (convicción media)"
            if (p <= 0.40) and (sem == "🔴"):
                return "SELL/REDUCE", "Prob baja + 🔴 (riesgo salida)"
            if (p <= 0.40):
                return "REDUCE", "Prob baja de expansión GAP"
            return "HOLD", "Prob media / transición"

        if sem == "🟢" and (pd.isna(fs) or fs >= 70):
            return "BUY", "🟢 + flujo alto"
        if sem == "🔴":
            return "REDUCE", "🔴"
    ...
    for paper in df_model["Nemo"].unique():
        ...
        for _, r in d.iterrows():
            ...
            if prev is None or fase != prev:
                if fase == "BUY (fuerte)":
                    nota = "Compra fuerte AFP: GAP sube y acelera."
                elif fase == "BUY (débil)":
                    nota = "Compra continúa pero pierde impulso."
                elif fase == "SELL":
                    nota = "Venta / descarga: ΔGAP < 0."
                elif fase == "UNDERWEIGHT (más)":
                    nota = "Aumenta infraponderación (negativo)."
                elif fase == "BUY (cover)":
                    nota = "Cubre infraponderación (reversión)."
                else:
                ...
                events.append({
                    ...
                    "FlowScore_0_100": r.get("FlowScore_0_100", np.nan),
...
def build_outputs(xls_source):
    ...
    # ✅ última fecha: estricta desde I2 si calza por año-mes; si no, cae al max()
