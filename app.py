...
def appendice_probabilidad():
    st.markdown(
        """
### Apéndice de probabilidad de compra AFP próximo mes
La **Probabilidad de compra AFP próximo mes** estima la probabilidad de que el **Delta GAP del próximo mes sea positivo**.

Rangos de lectura:
- **> 60%**: sesgo comprador
- **45% a 60%**: neutral / transición
- **< 40%**: sesgo vendedor

Modelo:
- Se estima usando el histórico por papel y variables como GAP, Delta GAP y cambios recientes del flujo.
"""
    )
...
- Combina:
  - semáforo
  - compra fuerte / venta fuerte
  - probabilidad de compra AFP próximo mes
...
**Cómo se calcula**
- Usa un score interno que mezcla:
  - posición actual
  - cambio del flujo
  - probabilidad próxima
"""
    )
...
with st.spinner("Procesando datos..."):
    df_raw, df_model, snap_last, metrics, last_date = None, None, None, None, None
    # compatibilidad si el pipeline devuelve 6 elementos
...
with tabs[1]:
    ...
    with st.expander("Apéndice — señales, probabilidad y acciones"):
        appendice_senales()
        appendice_probabilidad()
        appendice_acciones()
...
- **Top oportunidades** prioriza:
  ...
  2. probabilidad de compra AFP próximo mes
  3. Delta GAP
...
with tabs[3]:
    ...
    if "Prob_Compra_AFP_ProxMes" in d.columns:
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(x=d["Fecha"], y=d["Prob_Compra_AFP_ProxMes"], mode="lines", name="Prob. compra AFP próximo mes"))
        fig3.update_layout(
            template="plotly_white",
            hovermode="x unified",
    ...
    show_e = ["Fecha", "Semaforo", "Fase", "Flujo_AFP", "CompraVenta_Fuerte", "GAP", "Delta_GAP", "Prob_Compra_AFP_ProxMes", "Nota"]
    ...
    if len(pe) >= 1:
        ...
        fig_t.add_trace(go.Scatter(
            ...
            hovertemplate=
                ...
                "Delta GAP: %{customdata[4]:.2%}<br>" +
                "Prob compra próximo mes: %{customdata[5]:.0%}<extra></extra>",
            customdata=pe_plot[["Fase", "Flujo_AFP", "CompraVenta_Fuerte", "GAP", "Delta_GAP", "Prob_Compra_AFP_ProxMes"]].values
        ))
